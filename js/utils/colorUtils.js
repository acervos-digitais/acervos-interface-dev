...
function mean_std(arr) {
  ...
  const mu = arr.reduce((sum, x) => sum + x, 0) / arr.length;
  const sigsq = arr.reduce((sum, x) => sum + (x - mu) ** 2, 0) / arr.length;
  return [mu, sigsq ** 0.5];
}
...
function activationsToImage(activations2D) {
  ...
  const [mu, sigma] = mean_std(activations);
  const s1 = mu + sigma;
  const s2 = s1 + sigma;
  ...
  activations.forEach((act, px) => {
    ...
    data[4 * px + 1] = 0;
    data[4 * px + 2] = 0;
    data[4 * px + 3] = (act > s2) ? 0 : (act > s1) ? 85 : 255;
  });
